...
flag_validation = 0
#flag_validation = 0:
validation_dataset = "sim10shots3"
# validation_dataset = "sim_first"
...
def shooting(nn_model, t, t_np, ang_v, quaternions, actuation, just_last_sample, rtol_aux, atol_aux, shooting_time, ang_v_loss_hparam, t_separation, physical_params, lambda_nn_reg):
    ...
    def f_wrapper(t, x, physical_params, nn_model, u):
        ...
        R_flat = R.view(-1)
        x_in = torch.cat([R_flat, ang_v.view(-1), u.view(-1)], dim=0)  # single input vector

        quat_d = 0.5 * torch.matmul(Omega(ang_v), quat)   # always from MB
        ang_v_d_residual = nn_model(x_in)                 # only residual on ang_v
        ang_v_d_mb = f_mb(t, x, physical_params, u)[4:]   # model-based ang_v dynamics
        ang_v_d = ang_v_d_mb + ang_v_d_residual           # corrected angular accel
        dxdt = torch.cat([quat_d.view(-1), ang_v_d.view(-1)])

        # Save the residual dynamics and the MB dynamics for loss penalization
        nn_residual_norms.append(torch.norm(ang_v_d_residual))
    ...
    for chunk in range(len(shooting_intervals)):
        ...
        predicted_next_states_shoot_normalized = []
        for i in range(predicted_next_states_shoot.shape[0]):
            new_state = predicted_next_states_shoot[i, :]
            predicted_next_states_shoot_normalized.append(new_state) # Normalize the quaternion to use in the loss computation
            pred_quaternions.append(predicted_next_states_shoot[i, :4])
            pred_ang_v.append(predicted_next_states_shoot[i,4:7])
        predicted_next_states_shoot = torch.stack(predicted_next_states_shoot_normalized)
    ...
    # Convert the list of predicted quaternions and angular velocities to numpy arrays
    pred_quaternions = torch.stack(pred_quaternions).detach().cpu().numpy()
    pred_ang_v = torch.stack(pred_ang_v).detach().cpu().numpy()
...
def split_val_train_chunks(t, ang_v, quaternions, actuation, t_separation, shooting_time, percentage_validation, percentage_consider_chunk, flag_validation):
    segments = []
    start_idx = 0
    ...
    # Split data
    t_train, ang_v_train, quaternions_train, actuation_train = [], [], [], []
    t_val, ang_v_val, quaternions_val, actuation_val = [], [], [], []
    ...
    for i, (start, end) in enumerate(segments):
        ...
        target_tensors[1].append(ang_v[start:end])
        target_tensors[2].append(quaternions[start:end])
        target_tensors[3].append(actuation[start:end])
...
def plot_shooting_results_rmse(datasets_vec, plot_select_data_vec, plot_t_select_data_initial_vec, plot_t_select_data_final_vec, plot_t_u_flag_vec, plot_t_u_before_vec, plot_t_u_after_vec, nn_model, ang_v_loss_hparam, pred_samples_factor, shooting_time, just_last_sample, rtol_aux, atol_aux, file, directory1, directory2, directory3, num_epochs, lr, optimizer, flag_scheduler, str_t_u_flag, folder_gimble, physical_params, lambda_nn_reg):
    ...
    for i in range(len(datasets_vec)):
        ...
        # Shooting
        loss,pred_quaternions,pred_ang_v,_ = shooting(nn_model, t, t_np, ang_v, quaternions, actuation, just_last_sample, rtol_aux, atol_aux, shooting_time, ang_v_loss_hparam, t_separation, physical_params, lambda_nn_reg)
        ...
        # Plot quaternions norm across time
...
if __name__ == "__main__":
    ...
    physical_params = torch.tensor(initial_params, dtype=torch.float32, requires_grad=True)

    # Initialize the NN model
    if nn_model_id == 1:
        nn_model = DynamicsMLP1(input_dim=18, hidden_dim=hidden_dim, output_dim=3)
    elif nn_model_id == 2:
        nn_model = DynamicsMLP2(input_dim=18, hidden_dim=hidden_dim, output_dim=3)
    elif nn_model_id == 3:
        nn_model = DynamicsMLP3(input_dim=18, hidden_dim=hidden_dim, output_dim=3)
    elif nn_model_id == 4:
        nn_model = DynamicsMLP4(input_dim=18, hidden_dim=hidden_dim, output_dim=3)
    elif nn_model_id == 5:
        nn_model = DynamicsMLP5(input_dim=18, hidden_dim=hidden_dim, output_dim=3)
    elif nn_model_id == 6:
        nn_model = DynamicsMLP6(input_dim=18, hidden_dim=hidden_dim, output_dim=3)
    else:
    ...
    if flag_validation == 0: #Load new dataset for validation
        t_validation, ang_v_validation, quaternions_validation, actuation_validation = read_data(validation_file_path)
        ...
        data = split_val_train_chunks(t, ang_v, quaternions, actuation, t_separation, shooting_time, percentage_validation, percentage_consider_chunk, flag_validation)
    ...
    for epoch in range(num_epochs):
        ...
        total_loss,_,_,odeint_time = shooting(nn_model, t, t_np, ang_v, quaternions, actuation, just_last_sample, rtol_aux, atol_aux, shooting_time, ang_v_loss_hparam, t_separation, physical_params, lambda_nn_reg)
        ...
        validation_loss,_,_,_ = shooting(nn_model, t_validation, t_validation_np, ang_v_validation, quaternions_validation, actuation_validation, just_last_sample, rtol_aux, atol_aux, shooting_time, ang_v_loss_hparam, t_separation, physical_params, lambda_nn_reg)
    ...
    final_loss,_,_,_ = shooting(nn_model, t, t_np, ang_v, quaternions, actuation, just_last_sample, rtol_aux, atol_aux, shooting_time, ang_v_loss_hparam, t_separation, physical_params, lambda_nn_reg)
    ...
    if best_model_state_dict is not None and best_valloss_model==1:
        ...
        if nn_model_id == 1:
            best_nn_model = DynamicsMLP1(input_dim=18, hidden_dim=hidden_dim, output_dim=3)
        elif nn_model_id == 2:
            best_nn_model = DynamicsMLP2(input_dim=18, hidden_dim=hidden_dim, output_dim=3)
        elif nn_model_id == 3:
            best_nn_model = DynamicsMLP3(input_dim=18, hidden_dim=hidden_dim, output_dim=3)
        elif nn_model_id == 4:
            best_nn_model = DynamicsMLP4(input_dim=18, hidden_dim=hidden_dim, output_dim=3)
        elif nn_model_id == 5:
            best_nn_model = DynamicsMLP5(input_dim=18, hidden_dim=hidden_dim, output_dim=3)
        elif nn_model_id == 6:
            best_nn_model = DynamicsMLP6(input_dim=18, hidden_dim=hidden_dim, output_dim=3)
        # Load the best model
    ...
    if best_model_state_dict is not None and best_valloss_model==1:
        ...
        # Plots the shooting results and compute RMSE results
        plot_shooting_results_rmse(datasets_vec, plot_select_data_vec, plot_t_select_data_initial_vec, plot_t_select_data_final_vec, plot_t_u_flag_vec, plot_t_u_before_vec, plot_t_u_after_vec, best_nn_model, ang_v_loss_hparam, pred_samples_factor, shooting_time, just_last_sample, rtol_aux, atol_aux, file, directory1, directory2, directory3, num_epochs, lr, optimizer, flag_scheduler, str_t_u_flag, folder_gimble, physical_params, lambda_nn_reg)
