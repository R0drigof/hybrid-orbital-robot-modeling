...
t_u_flag = 0 #0- no part is removed, 1- selected part is removed
t_u_before = 0.1
t_u_after = 0.1
...
def shooting(nn_model, t, t_np, ang_v, quaternions, actuation, just_last_sample, rtol_aux, atol_aux, shooting_time, ang_v_loss_hparam, t_separation):

    # Define the dynamics of the system
    def f_wrapper(t, x, nn_model, u):
        ...
        ang_v_d = nn_model(x_in)

        # quaternion derivative, calculated directly using the angular velocity one through kinematics
    ...
    for chunk in range(len(shooting_intervals)):
        ...
        for i in range(len(index_u_chunk_groups)):
            ...
            predicted_next_states_shoot_i = odeint(
                lambda t, x: f_wrapper(t, x, nn_model, actuation_value), 
                # f_wrapper,
                state,
...
def plot_shooting_results_rmse(datasets_vec, plot_select_data_vec, plot_t_select_data_initial_vec, plot_t_select_data_final_vec, plot_t_u_flag_vec, plot_t_u_before_vec, plot_t_u_after_vec, nn_model, ang_v_loss_hparam, pred_samples_factor, shooting_time, just_last_sample, rtol_aux, atol_aux, file, directory1, directory2, directory3, num_epochs, lr, optimizer, flag_scheduler, str_t_u_flag, folder_gimble):
    ...
    for i in range(len(datasets_vec)):
        ...
        # Shooting
        loss,pred_quaternions,pred_ang_v,_ = shooting(nn_model, t, t_np, ang_v, quaternions, actuation, just_last_sample, rtol_aux, atol_aux, shooting_time, ang_v_loss_hparam, t_separation)
...
if __name__ == "__main__":
    ...
    for epoch in range(num_epochs):
        ...
        total_loss,_,_,odeint_time = shooting(nn_model, t, t_np, ang_v, quaternions, actuation, just_last_sample, rtol_aux, atol_aux, shooting_time, ang_v_loss_hparam, t_separation)
        ...
        validation_loss,_,_,_ = shooting(nn_model, t_validation, t_validation_np, ang_v_validation, quaternions_validation, actuation_validation, just_last_sample, rtol_aux, atol_aux, shooting_time, ang_v_loss_hparam, t_separation)
    ...
    final_loss,_,_,_ = shooting(nn_model, t, t_np, ang_v, quaternions, actuation, just_last_sample, rtol_aux, atol_aux, shooting_time, ang_v_loss_hparam, t_separation)
